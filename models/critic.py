...
class Critic(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, hp):
        ...
        self.l1s = nn.Linear(state_dim,hp['num_hidden_critic_l1'])
        self.l1s_bn = nn.BatchNorm1d(hp['num_hidden_critic_l1'])
        self.l1s.weight.data = fanin_init(self.l1s.weight.data.size())


        self.l1a = nn.Linear(action_dim,hp['num_hidden_critic_l1'])
        self.l1a.weight.data = fanin_init(self.l1a.weight.data.size())


        self.l4 = nn.Linear(2 * hp['num_hidden_critic_l1'],hp['num_hidden_critic_l2'])
    ...
    def predict(self, state, action):
        ...
        #out_s = F.relu(self.l1s_bn(self.l1s(state))) # With Batch normalization
        out_s = F.relu((self.l1s(state)))             # Without BatchNormalization
